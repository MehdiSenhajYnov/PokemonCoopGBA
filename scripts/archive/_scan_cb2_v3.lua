--[[
  Autonomous scanner v3: Uses frame callback to wait for ROM to be loaded.
  Also loads save state to ensure we're in-game.
]]
...
writeFile(PROGRESS, "waiting_for_frames")
...
-- ============================================================
-- Wait for ROM to be loaded (60 frames = ~1 second), then load save state + scan
-- ============================================================
local frameCount = 0
...
frameCb = callbacks:add("frame", function()
  ...
  -- At frame 10, load save state
  if frameCount == 10 then
    writeFile(PROGRESS, "loading_savestate")
    pcall(function() emu:loadStateSlot(1) end)
    return
  end

  -- Wait 120 frames after save state load for stabilization
  if frameCount < 130 then return end
  ...
  -- Verify ROM is loaded by reading known address
  local ok1, cb2 = pcall(emu.memory.wram.read32, emu.memory.wram, 0x2064C)
  if not ok1 then
    writeFile(OUTPUT, '{"status":"error","error":"wram read failed after 130 frames"}')
    writeFile(PROGRESS, "error")
    return
  end

  -- Quick sanity check: callback2 should be a ROM pointer on the overworld
  local sanity = string.format("callback2=0x%08X", cb2)
  ...
  log(string.format("Found %d refs to 0x%08X", #handleStartRefs, CB2_HANDLE_START))
  ...
  for i, c in ipairs(ibIntCandidates) do
    log(string.format("  IBI Candidate %d: 0x%08X (%d bytes, %d BLs)", i, c.addr, c.size, c.blCount))
  end
  if #ibIntCandidates > 0 then
    ...
    log("WARNING: No CB2_InitBattleInternal candidates found")
  end
  ...
  if cb2IntAddr then
    ...
    log(string.format("Found %d literal refs to CB2_InitBattleInternal", #intRefs))
    ...
    for i, c in ipairs(ibCandidates) do
      log(string.format("  IB Candidate %d: 0x%08X (%d bytes, %d BLs)", i, c.addr, c.size, c.blCount))
  ...
  if #ibCandidates > 0 then
    ...
    log("WARNING: No CB2_InitBattle candidates found")
  end
  ...
  for i, l in ipairs(logs) do
    json = json .. '    "' .. l:gsub('\\', '\\\\'):gsub('"', '\\"') .. '"'
    if i < #logs then json = json .. ',' end
    json = json .. '\n'
  end
  json = json .. '  ]\n'
  json = json .. '}\n'
